		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == customButton01) {
				...
				if (response == 0) {
					...
					textLabel.setText("custom01ダイアログボックスが閉じられました(選択：え)");
				} else if (response == 4) {
					textLabel.setText("custom01ダイアログボックスが閉じられました(選択：お)");
				}
			}

			// custom02
			if (e.getSource() == customButton02) {
				String str = (String) JOptionPane.showInputDialog(panel, "プルダウンメニュー", "カスタマイズ02",
						JOptionPane.INFORMATION_MESSAGE, null, new Object[] { "あ", "い", "う", "え", "お" }, "え");

				if (str == null) {
					str = "取り消し";
				}
				;
				textLabel.setText("カスタマイズ02ダイアログボックスが閉じられました(選択：" + str + ")");

			}

			// custom03
			if (e.getSource() == customButton03) {
				String str = (String) JOptionPane.showInputDialog(panel, "文字の入力を受け付けるよ", "カスタマイズ03",
						JOptionPane.INFORMATION_MESSAGE, null, null, "文字を入力してね");
				//取り消しを押すとnullが返る
				if (str == null) {
					str = "取り消し";
				};
				textLabel.setText("カスタマイズ03ダイアログボックスが閉じられました(選択：" + str + ")");
			}
		}
	}
}